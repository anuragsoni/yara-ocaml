...
static struct custom_operations yara_compiler_stub = {
    "yara_stubs_compiler",
    [](value compiler) { yr_compiler_destroy(Yr_compiler_val(compiler)); },
...
CAMLprim value yara_stubs_create_compiler(value unit) {
  ...
  CAMLlocal1(v);
  v = caml_alloc_custom_mem(&yara_compiler_stub, sizeof(YR_COMPILER *),
                            sizeof(YR_COMPILER));
  Yr_compiler_val(v) = compiler;
  CAMLreturn(v);
}

CAMLprim value yara_stubs_compiler_add_string(value rule_namespace,
                                              value compiler, value rule) {
  ...
  auto thunk = [](int error_level, const char *file_name, int line_number,
    ...
    error_level_ = error_level == YARA_ERROR_LEVEL_ERROR
                       ? caml_copy_string("error")
                       : caml_copy_string("warning");
  ...
  caml_release_runtime_system();
  int error_count = yr_compiler_add_string(
      Yr_compiler_val(compiler), String_val(rule),
      (rule_namespace == Val_none) ? NULL
                                   : String_val(Some_val(rule_namespace)));
  caml_acquire_runtime_system();
...
static struct custom_operations yara_rules_stub = {
    "yara_stubs_rules",
    [](value rules) { yr_rules_destroy(Yr_rules_val(rules)); },
    custom_compare_default,
    custom_hash_default,
    custom_serialize_default,
    custom_deserialize_default,
...
CAMLprim value yara_stubs_get_rules(value compiler) {
  ...
  CAMLlocal1(v);
  v = caml_alloc_custom_mem(&yara_rules_stub, sizeof(YR_RULES *),
                            sizeof(YR_RULES));
...
static struct custom_operations yara_scanner_stub = {
    "yara_stubs_scanner",
    [](value scanner) { yr_scanner_destroy(Yr_scanner_val(scanner)); },
    custom_compare_default,
    custom_hash_default,
    custom_serialize_default,
    custom_deserialize_default,
...
CAMLprim value yara_stubs_scanner_create(value rules) {
  ...
  CAMLlocal1(v);
  v = caml_alloc_custom_mem(&yara_scanner_stub, sizeof(YR_SCANNER *),
                            sizeof(YR_SCANNER));
...
CAMLprim value yara_stubs_scanner_set_int_variable(value scanner,
                                                   value identifier, value v) {
  CAMLparam3(scanner, identifier, v);
  auto result = yr_scanner_define_integer_variable(
      Yr_scanner_val(scanner), String_val(identifier), Int64_val(v));
  if (result != ERROR_SUCCESS) {
    yara_stubs_raise(result, __FUNCTION__);
  }
  CAMLreturn(Val_unit);
...
CAMLprim value yara_stubs_scanner_set_bool_variable(value scanner,
                                                    value identifier, value v) {
  CAMLparam3(scanner, identifier, v);
  auto result = yr_scanner_define_boolean_variable(
      Yr_scanner_val(scanner), String_val(identifier), Bool_val(v));
  if (result != ERROR_SUCCESS) {
    yara_stubs_raise(result, __FUNCTION__);
  }
  CAMLreturn(Val_unit);
...
CAMLprim value yara_stubs_scanner_set_float_variable(value scanner,
                                                     value identifier,
                                                     value v) {
  CAMLparam3(scanner, identifier, v);
  auto result = yr_scanner_define_float_variable(
      Yr_scanner_val(scanner), String_val(identifier), Double_val(v));
  if (result != ERROR_SUCCESS) {
    yara_stubs_raise(result, __FUNCTION__);
  }
  CAMLreturn(Val_unit);
...
CAMLprim value yara_stubs_scanner_set_string_variable(value scanner,
                                                      value identifier,
                                                      value v) {
  CAMLparam3(scanner, identifier, v);
  auto result = yr_scanner_define_string_variable(
      Yr_scanner_val(scanner), String_val(identifier), String_val(v));
  if (result != ERROR_SUCCESS) {
    yara_stubs_raise(result, __FUNCTION__);
  }
  CAMLreturn(Val_unit);
...
CAMLprim value yara_stubs_scanner_get_rules_matching(value scanner, value buf,
                                                     value pos, value len) {
  ...
  auto thunk = [](YR_SCAN_CONTEXT *context, int message, void *message_data,
                  void *user_data) -> int {
    caml_acquire_runtime_system();
  ...
  yr_scanner_set_flags(Yr_scanner_val(scanner),
                       SCAN_FLAGS_REPORT_RULES_MATCHING);

  caml_release_runtime_system();
  auto *buffer = static_cast<const uint8_t *>(Caml_ba_data_val(buf)) + pos;
  int result = yr_scanner_scan_mem(Yr_scanner_val(scanner), buffer,
